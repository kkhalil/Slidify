...
lm1 <- lm(eruptions ~ waiting, data=trainFaith)
pv <- predict(lm1,data.frame(waiting=65))
trRMSE <- sqrt(sum((lm1$fitted-trainFaith$eruptions)^2))
teRMSE <- sqrt(sum((predict(lm1, newdata=testFaith)-testFaith$eruptions)^2))
```
The predicted duration of 65 is  `r pv`
The RMSE on training set is `r trRMSE` and the RMSE on testing set is `r teRMSE`
```{r,echo = FALSE}
plot(trainFaith$waiting, trainFaith$eruptions, pch=19, col="blue", xlab="Waiting", ylab="Duration")
lines(trainFaith$waiting, lm1$fitted, col= 'red', lwd = 3)          
```
